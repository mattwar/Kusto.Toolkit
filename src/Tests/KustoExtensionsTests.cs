...
namespace Tests
{
    ...
    public class KustoExtensionsTests
    {
        ...
        public void TestGetSourceColumns()
        {
            ...
            TestGetSourceColumns("TabXY | project x", "TabXY.x", globals);

            // projected and renamed column can be tracked back to its source
            TestGetSourceColumns("TabXY | project Q=x", "TabXY.x", globals);

            // unioned and renamed columns can be tracked back to their source
            TestGetSourceColumns("TabXY | union TabXZ | project x", "TabXY.x, TabXZ.x", globals);
            TestGetSourceColumns("TabXY | union TabXZ | project Q=x", "TabXY.x, TabXZ.x", globals);

            // joined columns can be tracked back to their source
            TestGetSourceColumns("TabXY | join TabXZ on x | project Q=x", "TabXY.x", globals, includeIncidentalColumns: false);
            TestGetSourceColumns("TabXY | join TabXZ on x | project x1", "TabXZ.x", globals, includeIncidentalColumns: false);

            // columns generated by aggregates can be tracked back to their source
            TestGetSourceColumns("TabXY | summarize min(x)", "TabXY.x", globals);
            ...
            TestGetSourceColumns("FnQ", "TabXY.x", globals);
            TestGetSourceColumns("FnPQ", "TabXY.x", globals);
        }

        private static void TestGetSourceColumns(string query, string columnNames, GlobalState globals, bool includeIncidentalColumns=true)
        {
            TestGetColumns(query, columnNames, globals, code =>
            {
                return code.GetSourceColumns(includeIncidentalColumns);
            });
        }

        private static void TestGetColumns(
            string query, string columnNames, GlobalState globals, 
            Func<KustoCode, IReadOnlyList<ColumnSymbol>> fnGetColumns)
        {
            ...
            var expectedColumns = GetColumns(columnNames, globals);
